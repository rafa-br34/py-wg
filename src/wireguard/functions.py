...
def wg_aead_encrypt(key: bytes, idx: int, msg: bytes, associated_data: bytes) -> bytes:
	return chacha20poly1305_encrypt(msg, associated_data, b"\x00\x00\x00\x00" + idx.to_bytes(8, "little", signed = False), key)

def wg_aead_decrypt(key: bytes, idx: int, msg: bytes, associated_data: bytes) -> bytes:
	return chacha20poly1305_decrypt(msg, associated_data, b"\x00\x00\x00\x00" + idx.to_bytes(8, "little", signed = False), key)
